...
rather than discovered after a patient is affected. \autoref{fig:fear} draws the
...
\node[nfrisk] (b) at (3.5,0) {Error compounds};
\node[nfrisk] (c) at (7.0,0) {Harm reaches patient};
\node[nftwo] (d) at (1.7,-2.1) {Generate};
\node[nfdec] (e) at (5.2,-2.1) {Gate};
\node[nfhope] (f) at (8.6,-2.1) {Defect contained};
\draw[nfedge] (a)--(b); \draw[nfedge] (b)--(c);
\draw[nfedge] (d)--(e); \draw[nfedge] (e)--(f);
\end{narrativefig}
\vspace{-0.15cm}
\figcaption{Figure 4. Compounding error, above, versus a contained defect, below
(Mermaid 04 and 20 as colored TikZ). The gate stops a defect at its source.}
...
\bottomrule
\end{tabularx}
\caption{Table 2. The legacy path and the verified path, step by step.}
\label{tab:fear}
\end{table}
