...
\node[nftwo] (b) at (3.6,0) {Claude Code generate};
\node[nftwo] (c) at (7.2,0) {Codex review};
\node[nfdec] (d) at (10.5,0) {VVUQ ten gates};
...
\draw[nfedge] (d) to[out=-120,in=60] (acc.north);
\draw[nfedge] (d) -- (esc.north);
\draw[nfedge] (d) to[out=-60,in=120] (blk.north);
\draw[nfedged] (esc.south) to[out=-90,in=-90,looseness=1.2] (b.south);
\end{narrativefig}
\vspace{-0.15cm}
\figcaption{Figure 1. Verification before generation (Mermaid 01 reproduced as
colored TikZ). A human specification is drafted by one agent, reviewed by a
...
compassion, to the closing appeal, urgency, all converging on a single action.
...
\node[nftwo,text width=16mm]  (p5) at (9.0,-2.0){5\\Duty};
\node[nftwo,text width=16mm]  (p6) at (6.0,-2.0){6\\Protect};
\node[nfhope,text width=16mm] (p7) at (3.0,-2.0){7\\Trust};
\node[nfrisk,text width=16mm] (p8) at (0,-2.0) {8\\Urgency};
\node[nfact,text width=22mm]  (ax) at (4.5,-3.9){Enact H. R. 9510};
...
\end{narrativefig}
\vspace{-0.15cm}
\figcaption{Figure 2. The eight emotional pillars (Mermaid 02 reproduced as
colored TikZ). The argument accumulates from compassion to urgency and converges
