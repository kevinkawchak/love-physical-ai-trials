...
\node[nftwo] (b) at (4,0) {Informed\\consent};
\node[nfact] (c) at (8,0) {Plan verified\\by gates};
\node[nftwo] (d) at (12,0) {Procedure\\under audit};
\node[nfhope] (e) at (8,-1.7) {Transparent\\outcome};
\draw[nfedge] (a)--(b); \draw[nfedge] (b)--(c); \draw[nfedge] (c)--(d);
\draw[nfedge] (d) to[out=-90,in=0] (e.east);
\end{narrativefig}
\vspace{-0.75cm}
\figcaption{Figure 3. A patient's journey through a verified trial: consent and verification precede any action, and the outcome is
...
\footnotesize
\begin{tabularx}{\textwidth}{@{}L{4cm} Y L{5.3cm}@{}}
\toprule
