...
trusted. Figure 9 names the supports that hold that trust up.

\begin{narrativefig}
\node[nfact] (r) at (0,0) {Trust in medical AI};
\node[nftwo] (a) at (5,1.5) {Physician endorsement};
\node[nftwo] (b) at (5,0.0) {Independent review};
\node[nftwo] (c) at (5,-1.5) {Ten VVUQ gates};
\node[nfhope] (d) at (10,1.5) {Transparency};
\node[nfhope] (e) at (10,0.0) {Pilot results};
\node[nfhope] (f) at (10,-1.5) {Audit trail};
\draw[nfedge] (r.north east)--(a.south west); \draw[nfedge] (r.east)--(b.west); \draw[nfedge] (r.south east)--(c.north west);
\draw[nfedge] (a)--(d); \draw[nfedge] (b)--(e); \draw[nfedge] (c)--(f);
\end{narrativefig}
\vspace{-0.55cm}
\figcaption{Figure 9. Trust scaffolding: confidence
...
Kawchak2026VVUQOncologyClinicalTrial}. Figure 10 shows the funnel,
and \autoref{tab:trust} binds representative gates to their standards and
thresholds, which is what makes the assurance argument traceable.

\vspace{-0.1cm}

\begin{narrativefig}
\node[nfone] (i) at (0,0) {Candidate action};
\node[nftwo] (g1) at (4,0) {Verification fraction 1.0};
\node[nftwo] (g2) at (8,0) {Validation agreement};
\node[nfthree] (g3) at (12,0) {Standards binding};
...
\draw[nfedge] (g3) to[out=-120,in=70, looseness=0.6] (g4.east);
\draw[nfedge] (g4)--(o);
\end{narrativefig}
\vspace{-0.55cm}
\figcaption{Figure 10. The ten-gate funnel: each gate
is a published threshold, and only a clean pass reaches ACCEPT.}

\needspace{9\baselineskip}\begin{table}[H]
\footnotesize
\begin{tabularx}{\textwidth}{@{}L{5.6cm} Y L{3.0cm}@{}}
\toprule
...
\bottomrule
\end{tabularx}
\caption{Representative gates bound to standards and thresholds.}
\label{tab:trust}
\end{table}
