...
entrepreneurship that pairs a problem with a demonstrated solution. Figure 6 shows the lineage on which the hope
rests.

\begin{narrativefig}
\node[nfone] (a) at (0,0) {LLM applications};
\node[nftwo] (b) at (4,0) {VVUQ pipeline};
\node[nftwo] (c) at (8,0) {Humanoid, 172 tests};
\node[nfthree] (d) at (4,-2.1) {National platform};
\node[nfact] (e) at (8,-2.1) {H. R. 9510 v5.0};
\node[nfhope] (f) at (11.5,-1.05) {Lives saved};
\draw[nfedge] (a)--(b); \draw[nfedge] (b)--(c);
\draw[nfedge] (c) to[out=-90,in=70, looseness=0.4] (d.north);
\draw[nfedge] (d)--(e); \draw[nfedge] (e) to[out=0,in=210, looseness=1] (f.south);
\end{narrativefig}
\vspace{-0.75cm}
\figcaption{Figure 6. Hope grounded in a documented lineage: each step is a real deposit on which the next was built.}
...
\bottomrule
\end{tabularx}
\caption{Four hopeful outcomes and the provisions that enable them.}
\label{tab:hope}
\end{table}
