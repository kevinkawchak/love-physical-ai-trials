...
Figure 7 shows the path on which these actions operate.

\begin{narrativefig}
\node[nfone] (a) at (0,0) {Introduce};
\node[nftwo] (b) at (4,0) {Hearings, testimony};
\node[nfact] (c) at (8,0) {Markup};
\node[nftwo] (d) at (12.0,0) {Floor passage};
\node[nfthree] (e) at (4.0,-2.1) {Reconciliation};
\node[nfhope] (f) at (8,-2.1) {Enacted law};
\draw[nfedge] (a)--(b); \draw[nfedge] (b)--(c); \draw[nfedge] (c)--(d);
\draw[nfedge] (d) to[out=-90,in=70, looseness=0.4] (e.north);
\draw[nfedge] (e)--(f);
\draw[nfedged] (c) to[out=-120,in=280, looseness=0.6] (b.south);
\end{narrativefig}
\vspace{-0.75cm}
\figcaption{Figure 7. The duty to move a bill forward, with testimony feeding markup and
reconciliation joining two chambers.}

\clearpage

\autoref{tab:duty} maps main actions to the committee step, making where to apply effort apparent.
...
\bottomrule
\end{tabularx}
\caption{Each main action and the legislative step it serves.}
\label{tab:duty}
\end{table}
