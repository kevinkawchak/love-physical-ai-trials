...
\cite{Kawchak2026PatientPriorityProposedU}. Figure 5 shows how a
documented gap becomes an audited safeguard.

\begin{narrativefig}
\node[nfrisk] (a) at (0,0) {Unequal\\access};
\node[nfrisk] (b) at (4,0) {Bias\\encoded};
\node[nfdec] (c) at (8,0) {Bias\\survey};
\node[nfact] (d) at (12,0) {Measured and\\reported};
\node[nfhope] (e) at (12,-2.1) {Equal\\protection};
\draw[nfedge] (a)--(b); \draw[nfedge] (b)--(c); \draw[nfedge] (c)--(d);
\draw[nfedge] (d)--(e);
\end{narrativefig}
\vspace{-0.8cm}
\figcaption{Figure 5. From a documented disparity to an audited safeguard. Surveillance turns an unfair gap into a measured,
correctable number.}
...
\bottomrule
\end{tabularx}
\caption{Turning a documented injustice into a statutory check.}
\label{tab:outrage}
\end{table}
